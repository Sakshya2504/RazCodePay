...
export function registerIntegrationRoutes(app) {
  ...
  router.get('/razorpay', async (req, res) => {
    ...
    return res.json({ connected: Boolean(connection), mode: connection?.mode || null, keyId: connection?.keyId || null, status: connection?.status || null, webhookConfigured: Boolean(connection?.encryptedWebhookSecret), lastVerifiedAt: connection?.lastVerifiedAt || null });
  });
  ...
  router.post('/razorpay', requireRole('owner', 'admin'), async (req, res, next) => {
    try {
      ...
      await saveRazorpayConnection(req.auth.merchantId, { keyId, encryptedSecret: encryptSecret(keySecret), ...(webhookSecret ? { encryptedWebhookSecret: encryptSecret(webhookSecret) } : {}), mode, status: 'connected', connectedAt: new Date() });
      ...
      return res.json({ connected: true, mode, keyId, webhookConfigured: Boolean(webhookSecret), message: 'Razorpay credentials verified and encrypted at rest.' });
    } catch (error) {
