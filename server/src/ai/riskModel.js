const WEIGHTS = { amount: 0.22, failureRecoverability: 0.26, freshness: 0.16, customerIntent: 0.14, consent: 0.08, priorAttemptsPenalty: -0.14 };
const FAILURE_PRIORS = { BAD_REQUEST_ERROR: 0.72, GATEWAY_ERROR: 0.62, NETWORK_ERROR: 0.76, CUSTOMER_ACTION_REQUIRED: 0.88, PAYMENT_FAILED: 0.68, AUTHENTICATION_ERROR: 0.48, SERVER_ERROR: 0.61 };
...
export function buildFeatures(caseData, now = new Date()) {
  ...
  const prior = FAILURE_PRIORS[failureCode] ?? 0.58;
  ...
  const customerIntent = caseData.type === 'checkout_abandonment' ? 0.82 : failureCode === 'CUSTOMER_ACTION_REQUIRED' ? 0.9 : 0.68;
...
export function scoreRecoveryPotential(caseData, now = new Date()) {
  ...
  const linear = f.prior * WEIGHTS.failureRecoverability + f.freshness * WEIGHTS.freshness + f.customerIntent * WEIGHTS.customerIntent + f.consent * WEIGHTS.consent + (1 - f.amount) * WEIGHTS.amount + Math.max(0, 1 - f.attempts) * 0.16 + Math.max(0, f.attempts - 1) * WEIGHTS.priorAttemptsPenalty;
  const recoverability = clamp(sigmoid((linear - 0.45) * 6));
  const risk = clamp(0.36 + f.amount * 0.34 + (1 - f.prior) * 0.2 + f.attempts * 0.08);
  return {
    riskScore: Number(risk.toFixed(3)), recoverabilityScore: Number(recoverability.toFixed(3)), modelVersion: 'local-recovery-v1', features: f,
    signals: [
